<?php
    ...
    function AddJoueur(){
        ...
        echo json_encode($response); //TODO : il aurait été intéressant que ces fonctions retourne $response sans utilisées echo (les fonctions pourraient alors être réutlisées dans un autre cadre qu'une API)
    }
    ...
    function UppJoueur(){
        ...
        if($stmt->execute()){
            $response=array(
                'status'=> 1,
                'status_message' => 'Joueur modifie avec succes'
            );
        } else {
            $response=array(
                'status' => 0,
